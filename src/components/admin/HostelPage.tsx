...
export default function HostelLanding() {
  ...
  const fetchHostelDataForThatDay = async () => {
    try {
        const response = await axios.get(`${baseURL}/api/feedback/get`);
        const response2 = await axios.get(`${baseURL}/api/feedback/get/${response.data.createdAt}`);
        setHostelData(response2.data.data)
    } catch (error) {
        console.error("Error fetching hostel data:", error);
    }
  }
